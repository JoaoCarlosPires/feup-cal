...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
    Vertex<T>* newV = findVertex(sourc);
...
bool Graph<T>::removeEdge(const T &sourc, const T &dest) {
    ...
    return newV->removeEdgeTo(findVertex(dest));
}
...
bool Vertex<T>::removeEdgeTo(Vertex<T> *d) {
    typename vector<Edge<T> >::const_iterator itr = this->adj.begin();
    for (itr; itr != this->adj.end(); itr++) {
        if ((*itr).dest == d) {
            this->adj.erase(itr);
            return true;
        }
...
bool Graph<T>::removeVertex(const T &in) {
    ...
    Vertex<T> * newV = new Vertex<T>(in);
	typename vector<Vertex<T> *>::const_iterator itr = this->vertexSet.begin();
    typename vector<Vertex<T> *>::const_iterator itr2 = this->vertexSet.begin();
	for (itr; itr != this->vertexSet.end(); itr++) {
	    if ((*itr)->info == in) {
            to_return = true;
	...
	if (to_return) {
	    for (itr2; itr2 != this->vertexSet.end(); itr2++) {
	        (*itr2)->removeEdgeTo(newV);
	    }
    }
	return to_return;
}
...
vector<T> Graph<T>::dfs() const {
    ...
    for (Vertex<T>* v : this->vertexSet)
        v->visited = false;
        for (Vertex<T>* v2 : this->vertexSet)
            if (!v2->visited)
                dfsVisit(v2, res);
	return res;
}
